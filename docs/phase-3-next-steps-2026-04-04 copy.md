...
### Settings Page Design Foundation

The current Settings page uses these design patterns that must be carried through to Billing workspace:

**Tab Navigation**
- Horizontal tab bar below section heading
- Active tab indicator with underline
- Neutral gray color for inactive tabs (`text-gray-500`)
- Current active tab in primary color
- Example tabs: Integrations, Templates, Webhook Logs, Roles & Permissions, Dashboard, System Health

**Section Layout**
- Top heading with descriptive subtitle
- Cards/panels for logical content grouping
- Status indicators: Pass ✓, configured (warning), disabled, healthy, stale
- Refresh buttons on cards for data reload
- Consistent padding and spacing (appears to use Tailwind: `p-6`, `gap-6`)

**Card Components**
```
┌─────────────────────────────────────────┐
│ Card Title (bold, dark text)            │
│ Card description or field value         │
│                                         │
│ Status indicator (green/yellow/gray)    │
│ Details text (secondary color)          │
│                                         │ [Refresh button if needed]
└─────────────────────────────────────────┘
```

**Color Palette**
- Primary action: Teal/Green (#10b981 or similar)
- Success/Pass: Green
- Warning/Configured Disabled: Amber/Orange
- Neutral text: Gray-500 to Gray-900
- Background: White with subtle gray accents
- Borders: Light gray

**Typography**
- Page title: Large, bold (h1)
- Section title: Medium, bold (h2)
- Card title: Regular bold (h3)
- Description text: Gray-600, regular weight
- Status labels: Small, uppercase, semibold

### Billing Workspace Tab Structure (Following Current Pattern)
...
**Implementation Note:** Billing workspace should be a **sub-tab structure**, not replacing existing tabs. Users toggle into "Billing" tab, then see nested sub-tabs for different billing components.

### Component Structure & Styling

```
resources/js/components/billing/
├── BillingWorkspace.jsx (main container, handles sub-tabs)
├── ProvidersTab.jsx
...
└── shared/
    ├── BillingCard.jsx (status card wrapper)
    ├── BillingForm.jsx (form with existing styling)
    ├── CredentialInput.jsx (masked field component)
    ├── SchemaFieldRenderer.jsx
    ├── ValidationErrorDisplay.jsx
    ├── StatusBadge.jsx (Pass/Configured Disabled/Healthy)
    └── RefreshButton.jsx
```

### Reusable BillingCard Component

Follow this pattern for all provider, routing, and config cards:

```jsx
<BillingCard
  title="Provider Name"
  status="healthy"  // or "configured_disabled", "stale", "pass"
  details="Active configuration for Kenya market"
  onRefresh={() => refetchProvider()}
  isLoading={isLoading}
>
  {/* Card content */}
</BillingCard>
...
<div className="flex gap-8 border-b border-gray-200">
  <button className="py-3 px-0 text-sm font-medium text-gray-700 border-b-2 border-transparent hover:border-teal-500">
    Providers
  </button>
</div>
```
...
if (config('billing.workspace_enabled', false)) {
    // Show Billing workspace tab in Settings
}
```
...
- Status/health indicators on cards showing last-refresh timestamp
...
- Status cards show "View" button instead of "Edit"
