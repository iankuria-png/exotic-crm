    ...
    'recorders' => [
        Recorders\CacheInteractions::class => [
            ...
            'groups' => [
                ...
                '/^sb_resolve:\d+:\d+$/' => 'sb_resolve:*:*',
                '/^sb_failure:\d+$/' => 'sb_failure:*',
                '/^push_upload:[\w-]+$/' => 'push_upload:*',
                '/^push_upload:index$/' => 'push_upload:index',
                '/^crm:sync-sb-users$/' => 'crm:sync-sb-users',
            ],
        ],
        ...
        Recorders\Servers::class => [
            'server_name' => env('PULSE_SERVER_NAME', (string) env('APP_NAME', 'Exotic CRM') . '@' . gethostname()),
            'directories' => array_values(array_filter(explode(':', env('PULSE_SERVER_DIRECTORIES', implode(':', [
                base_path(),
                storage_path(),
            ]))))),
        ],
        ...
        Recorders\SlowJobs::class => [
            ...
            'threshold' => [
                '#^App\\\\Jobs\\\\RunSupportBoardSyncJob$#' => 2000,
                '#^App\\\\Jobs\\\\RunSbLeadImportJob$#' => 2500,
                '#^App\\\\Jobs\\\\ProcessPushUploadJob$#' => 5000,
                '#^App\\\\Jobs\\\\ExtractPushProfilesJob$#' => 5000,
        ...
        Recorders\SlowOutgoingRequests::class => [
            ...
            'threshold' => [
                '#^https?://[^/]+/wp-json/exotic-crm-sync/v1/.*$#' => env('PULSE_SLOW_OUTGOING_REQUESTS_WP_THRESHOLD', 1500),
            ...
            'groups' => [
                '#^https?://[^/]+/wp-json/exotic-crm-sync/v1/clients/\d+.*$#' => 'wordpress-sync/client',
                '#^https?://[^/]+/wp-json/exotic-crm-sync/v1/analytics/\d+.*$#' => 'wordpress-sync/analytics',
                '#^https?://[^/]+/wp-json/exotic-crm-sync/v1/.*$#' => 'wordpress-sync/*',
                '#^https?://([^/]+).*$#' => '\1',
            ],
        ],
