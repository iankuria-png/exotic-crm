...
return new class extends Migration
{
    ...
    private function reportableWhere(): string
    {
        ...
        if (Schema::hasColumn('payments', 'purpose')) {
            $clauses[] = "(payments.purpose IS NULL OR payments.purpose NOT IN ('wallet_topup', 'visitor_contact_unlock'))";
        }
    ...
    private function canonicalCurrencyExpression(): string
    {
        ...
        $xofMarket = $this->marketMatches(['Benin', 'Benin Republic', 'Burkina Faso', "Cote d'Ivoire", "Côte d'Ivoire", 'Guinea-Bissau', 'Guinea Bissau', 'Ivory Coast', 'Mali', 'Niger', 'Senegal', 'Togo', 'BEN', 'BFA', 'CIV', 'GNB', 'MLI', 'NER', 'SEN', 'TGO']);
        $xafMarket = $this->marketMatches(['Cameroon', 'Central African Republic', 'Chad', 'Congo', 'Equatorial Guinea', 'Gabon', 'Republic of the Congo', 'CMR', 'CAF', 'TCD', 'COG', 'GNQ', 'GAB']);
