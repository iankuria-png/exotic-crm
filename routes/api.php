...
use App\Http\Controllers\CRM\BioTextHealthController;
...
Route::middleware(['auth:sanctum', 'crm.session-token', 'crm.active', 'crm.impersonation'])->prefix('crm')->group(function () {
    ...
    Route::middleware('role:field_sales,admin')->prefix('field')->group(function () {
        ...
        Route::get('/commissions', [FieldSalesController::class, 'commissions']);
        Route::get('/clients/{client}/deposit-status', [FieldSalesController::class, 'depositStatus']);
        Route::post('/clients/{client}/activate-trial', [FieldSalesController::class, 'activateTrial']);
    });
    ...
    Route::middleware('role:admin,sub_admin')->group(function () {
        ...
        Route::put('/settings/field-sales', [FieldSalesController::class, 'updateSettings']);
    });

    // SEO Profile Optimization Engine
    Route::prefix('seo')->group(function () {
        Route::get('/provider-options', [SeoController::class, 'providerOptions']);
    ...
    Route::middleware('crm.ceo')->prefix('dashboard/ceo')->group(function () {
        ...
        Route::prefix('forecast')->group(function () {
            Route::get('/baseline', [ForecastController::class, 'baseline']);
            Route::get('/baseline/status', [ForecastController::class, 'baselineStatus']);
            Route::post('/compute', [ForecastController::class, 'compute']);
            Route::post('/solve', [ForecastController::class, 'solve']);
            Route::post('/narrate', [ForecastController::class, 'narrate']);
            Route::get('/scenarios', [ForecastController::class, 'scenarios']);
            Route::post('/scenarios', [ForecastController::class, 'saveScenario']);
            Route::get('/scenarios/{scenario}', [ForecastController::class, 'showScenario']);
            Route::patch('/scenarios/{scenario}', [ForecastController::class, 'renameScenario']);
            Route::delete('/scenarios/{scenario}', [ForecastController::class, 'deleteScenario']);
            Route::get('/scenarios/{scenario}/export', [ForecastController::class, 'exportScenario']);
        });
    });
    ...
    // Bio text — bios with garbled accents, lost characters or AI leftovers,
    // checked per market and repaired in WordPress. Admins only.
    Route::middleware('role:admin,sub_admin')->prefix('bio-text-health')->group(function () {
        Route::get('/', [BioTextHealthController::class, 'show']);
        Route::post('/scans', [BioTextHealthController::class, 'store']);
        Route::get('/scans/{scan}', [BioTextHealthController::class, 'showScan']);
        Route::get('/scans/{scan}/findings', [BioTextHealthController::class, 'findings']);
        Route::post('/scans/{scan}/repair', [BioTextHealthController::class, 'repair']);
        Route::post('/scans/{scan}/restore', [BioTextHealthController::class, 'restore']);
        Route::get('/scans/{scan}/export', [BioTextHealthController::class, 'export']);
    });
    ...
    Route::get('/clients/{client}/contact-unlocks', [ClientController::class, 'contactUnlocks'])->middleware('role:admin,sub_admin,sales,field_sales,marketing');
    ...
    Route::post('/clients/{client}/online-now', [ClientController::class, 'markOnlineNow'])->middleware('role:admin,sub_admin,sales,field_sales');
    ...
    Route::get('/clients/{client}/media/conversions', [ClientController::class, 'mediaConversions'])->middleware('role:admin,sub_admin,sales,field_sales,marketing');
    ...
    Route::post('/clients/{client}/login-as-client/debug', [ClientController::class, 'debugLoginAsClient'])->middleware('role:admin');
    ...
    Route::middleware('role:admin,sub_admin,sales,field_sales')->group(function () {
        ...
        Route::get('/lifecycle-sms/analytics/messages', [LifecycleSmsController::class, 'analyticsMessages']);
        ...
        Route::get('/clients/{client}/lifecycle-sms/history', [LifecycleSmsController::class, 'history']);
    ...
    Route::get('/settings/integrations/sms-provider/balances', [SettingsController::class, 'smsProviderBalances'])->middleware('role:admin,sub_admin');
    ...
    Route::get('/settings/integrations/pbn-sites', [PbnSiteController::class, 'index'])->middleware('role:admin,sub_admin,sales');
    Route::post('/settings/integrations/pbn-sites', [PbnSiteController::class, 'store'])->middleware('role:admin,sub_admin');
    Route::patch('/settings/integrations/pbn-sites/{pbnSite}', [PbnSiteController::class, 'update'])->middleware('role:admin,sub_admin');
    Route::post('/settings/integrations/pbn-sites/{pbnSite}/test-connection', [PbnSiteController::class, 'testConnection'])->middleware('role:admin,sub_admin');
    Route::get('/settings/integrations/pbn-sites/{pbnSite}/locations', [PbnSiteController::class, 'locations'])->middleware('role:admin,sub_admin,sales');
    Route::post('/settings/integrations/pbn-sites/{pbnSite}/preview', [PbnSiteController::class, 'preview'])->middleware('role:admin,sub_admin,sales');
    Route::post('/settings/integrations/pbn-sites/{pbnSite}/batches', [PbnSiteController::class, 'storeBatch'])->middleware('role:admin,sub_admin,sales');
    Route::get('/settings/integrations/pbn-seed-batches/{batch}', [PbnSiteController::class, 'showBatch'])->middleware('role:admin,sub_admin,sales');
    Route::post('/settings/integrations/pbn-seed-batches/{batch}/retry', [PbnSiteController::class, 'retryBatch'])->middleware('role:admin,sub_admin,sales');
