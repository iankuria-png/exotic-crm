...
const PROVIDER_DISPLAY = {
    openrouter: {
        label: 'OpenRouter',
        help: 'One API key for many models. Use the model ladder below to experiment safely.',
    },
...
export default function SeoEnginePanel() {
    ...
    const handleSave = () => {
        const payload = {
            ...
            providers: {
                openrouter: providerPayload(form.providers.openrouter),
                ...
                <div className="flex items-start justify-between gap-4 flex-wrap">
                    <div className="min-w-0 flex-1">
                        ...
                        <p className="mt-1 text-sm text-slate-600">
                            Generate SEO-optimised profile bios automatically using an LLM waterfall (OpenRouter → DeepSeek → Gemini → Claude → OpenAI)
            ...
            <section className="crm-surface p-6">
                ...
                <ol className="mt-4 space-y-2">
                    {form.providersOrder.map((p, i) => {
                        ...
                        const providerState = form.providers[p] || {};
                        const configured = !!providerState.apiKey || providerState.hasKey || envKeys[p];
                        return (
                            ...
                            <div key={provider} className="rounded-md border border-slate-200 p-4">
                                ...
                                <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-3">
                                    ...
                                    <ProviderModelField
                                        ...
                                        openRouterPresets={settingsQuery.data?.openrouter_model_presets || {}}
...
function ProviderModelField({ provider, value, fallbackModels = [], openRouterPresets = {}, onChange, onModelChainChange }) {
    if (provider === 'openrouter') {
        return (
            <OpenRouterModelField
                ...
                onModelChainChange={onModelChainChange}
            />
        );
    }
...
function OpenRouterModelField({ value, fallbackModels = [], presets = {}, onModelChainChange }) {
    const [customModel, setCustomModel] = useState('');
    const [search, setSearch] = useState('');
    const [sort, setSort] = useState('most-popular');
    const [showCatalog, setShowCatalog] = useState(false);
    const chain = normalizeModelChain([value, ...fallbackModels]);
    const displayChain = chain.length > 0 ? chain : [MODEL_PRESETS.openrouter[0].value];

    const catalogQuery = useQuery({
        queryKey: ['seo-openrouter-models', search.trim(), sort],
        queryFn: () => api.get('/crm/settings/seo-engine/models', {
            params: {
    ...
    const addModel = (model) => {
        const trimmed = String(model || '').trim();
        if (!trimmed || displayChain.includes(trimmed)) return;
        applyChain([...displayChain, trimmed]);
        setCustomModel('');
    };
    ...
    const removeModel = (model) => {
        const next = displayChain.filter((item) => item !== model);
        applyChain(next.length > 0 ? next : [MODEL_PRESETS.openrouter[0].value]);
    };

    const applyPreset = (models = []) => {
        applyChain(models);
    };
    ...
    return (
        <div className="sm:col-span-2">
            <div className="flex items-center justify-between gap-2">
                <label className="block text-xs font-medium text-slate-700">Model ladder</label>
                <button
                    type="button"
            ...
            <div className="mt-1 rounded-md border border-slate-200 bg-slate-50 p-3">
                    ...
                    <div className="grid grid-cols-1 gap-2 md:grid-cols-2 xl:grid-cols-3">
                        {presetEntries.map(([key, preset]) => (
                            <button
                                ...
                                className="rounded-md border border-slate-200 bg-white px-3 py-2 text-left transition hover:border-teal-300 hover:bg-teal-50"
                                title={(preset.models || []).join(' → ')}
                            >
                                <div className="text-xs font-semibold text-slate-900">{preset.label || key}</div>
                ...
                <div className="mt-3 flex flex-wrap gap-1.5">
                    {MODEL_PRESETS.openrouter.map((preset) => {
                        const active = displayChain.includes(preset.value);
                        return (
                            <button
                                key={preset.value}
                ...
                <ol className="mt-3 space-y-1.5">
                    {displayChain.map((model, index) => (
                        <li key={model} className="grid grid-cols-[72px_1fr_auto_auto_auto] items-center gap-2 rounded-md border border-slate-200 bg-white px-2 py-1.5">
                            <span className="text-[10px] font-semibold uppercase tracking-[0.08em] text-slate-500">
                                {index === 0 ? 'Primary' : `Fallback ${index}`}
                            </span>
                            <span className="min-w-0 truncate font-mono text-xs text-slate-800">{model}</span>
                            <button
                                type="button"
                                onClick={() => moveModel(index, 'up')}
                                disabled={index === 0}
                    ...
                    <div className="mt-4 border-t border-slate-200 pt-3">
                        <div className="grid grid-cols-1 gap-2 md:grid-cols-[1fr_190px]">
                            <input
                                ...
                                className="text-xs rounded-md border-slate-300 focus:border-teal-500 focus:ring-teal-500"
                            />
                            <select
                                value={sort}
                            ...
                            >
                                ...
                                <option value="pricing-low-to-high">Lowest price</option>
                        ...
                        <div className="mt-2 max-h-72 overflow-auto rounded-md border border-slate-200 bg-white">
                            {catalogQuery.isLoading ? (
                                <div className="p-3 text-xs text-slate-500">Loading OpenRouter models...</div>
                            ) : catalogQuery.isError || catalogQuery.data?.success === false ? (
                                <div className="p-3 text-xs text-rose-700">Could not load the OpenRouter model catalog.</div>
                            ) : catalogModels.length === 0 ? (
                                <div className="p-3 text-xs text-slate-500">No matching models found.</div>
                            ) : (
                                <ul className="divide-y divide-slate-100">
                                    {catalogModels.map((model) => {
                                        const active = displayChain.includes(model.id);
                                        return (
                                            <li key={model.id} className="grid grid-cols-[1fr_auto] gap-3 px-3 py-2">
                                                <div className="min-w-0">
                                                    <div className="truncate text-xs font-semibold text-slate-900">{model.name}</div>
                                                    ...
                                                    <div className="mt-1 flex flex-wrap gap-1.5 text-[10px] text-slate-500">
                                                        ...
                                                        <span>{formatModelPrice(model.pricing?.prompt)} in</span>
                                                        <span>{formatModelPrice(model.pricing?.completion)} out</span>
                                                    </div>
                                                </div>
                                                <button
                                                    type="button"
                                                    ...
                                                    disabled={active}
                                                    className="self-center rounded-md border border-slate-300 px-2.5 py-1 text-[11px] font-semibold text-slate-700 hover:bg-slate-50 disabled:opacity-40"
                                                >
                                                    {active ? 'Added' : 'Add'}
                                                </button>
                                            </li>
                ...
                <p className="mt-2 text-[11px] text-slate-500">
                    OpenRouter tries the first model, then each fallback. Its own provider routing can still recover from upstream outages.
                </p>
            </div>
        </div>
    );
...
function formatModelPrice(value) {
    ...
    if (!Number.isFinite(numeric)) return '$0/M';
    return `$${numeric.toFixed(numeric < 1 ? 3 : 2)}/M`;
}

function formatContext(value) {
    const numeric = Number(value || 0);
    if (!numeric) return 'context n/a';
