...
export default function McpWorkspacePanel({ userRole = '' }) {
    ...
    const aliasesQuery = useQuery({
        queryKey: ['mcp-staff-aliases'],
        queryFn: () => api.get('/crm/settings/mcp/staff-aliases').then((response) => response.data),
        enabled: panel === 'privacy' && canManage,
    });
    ...
    const editGrantMutation = useMutation({
        mutationFn: ({ tokenId, payload }) => api.patch(`/crm/settings/mcp/tokens/${tokenId}`, payload).then((response) => response.data),
        onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['mcp-tokens'] }); toast.success('Token grant updated. The existing secret still works.'); },
        onError: (error) => toast.error(error?.response?.data?.message || 'Could not update this token grant.'),
    });

    const aliasMutation = useMutation({
        mutationFn: ({ userId, agent_alias }) => api.put(`/crm/settings/mcp/staff-aliases/${userId}`, { agent_alias, active: true }).then((response) => response.data),
        onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['mcp-staff-aliases'] }); toast.success('Staff alias saved. MCP results update immediately.'); },
        onError: (error) => toast.error(error?.response?.data?.message || 'Could not save this staff alias.'),
    });
    ...
    return (
        <div className="space-y-4" data-testid="mcp-workspace">
            ...
            {panel === 'tokens' && canManageTokens ? (
                <Tokens tools={tools} tokens={tokensQuery.data?.tokens || []} isLoading={tokensQuery.isLoading} isError={tokensQuery.isError} mintMutation={mintMutation} revokeMutation={revokeMutation} editGrantMutation={editGrantMutation} />
            ) : null}
            {panel === 'tools' ? (
            ...
            {panel === 'privacy' ? (
                <Privacy settings={settings} tools={tools} canManage={canManage} isSaving={updateMutation.isPending} onSave={saveSettings} previewMutation={previewMutation} staff={aliasesQuery.data?.staff || []} aliasesLoading={aliasesQuery.isLoading} onSaveAlias={(payload) => aliasMutation.mutate(payload)} aliasSaving={aliasMutation.isPending} />
            ) : null}
            {panel === 'limits' ? (
...
function Tokens({ tools, tokens, isLoading, isError, mintMutation, revokeMutation, editGrantMutation }) {
    ...
    return (
        <div className="space-y-4">
            <Section title="Access tokens" description="Read-only credentials expire automatically. The plaintext token is shown once and cannot be recovered." action={<button type="button" onClick={() => setMintOpen(true)} className={primaryButtonClass}>Mint token</button>}>
                ...
                {isLoading ? <ListSkeleton rows={3} /> : isError ? <EmptyState title="Tokens are unavailable" message="Only administrators can inspect and manage MCP credentials." /> : filtered.length ? <div className="divide-y divide-slate-200">{filtered.map((token) => <TokenRow key={token.id} token={token} onEdit={() => setEditToken(token)} onRevoke={() => setRevokeToken(token)} />)}</div> : <EmptyState title="No matching tokens" message="Mint an expiring token for a client, then return here to monitor its use." />}
            </Section>
            ...
            <McpDialog open={Boolean(editToken)} title={`Edit ${editToken?.label || 'token'} grant`} onClose={() => setEditToken(null)}><TokenGrantEditor token={editToken} tools={tools} onCancel={() => setEditToken(null)} onSubmit={(payload) => editGrantMutation.mutate({ tokenId: editToken.id, payload }, { onSuccess: () => setEditToken(null) })} isPending={editGrantMutation.isPending} /></McpDialog>
...
function MintTokenForm({ tools, onCancel, onSubmit, isPending }) {
    ...
    const submit = (event) => {
        ...
        onSubmit({ label: label.trim(), ttl_days: Number(ttl), tools: scoped ? selectedTools : enabledTools.map((tool) => tool.name) });
    };
    return <form onSubmit={submit} className="space-y-4"><Field label="Token label" hint="Use a device or client name you will recognize later."><input required value={label} onChange={(event) => setLabel(event.target.value)} className={inputClass} placeholder="e.g. Ian MacBook" autoFocus /></Field><Field label="Lifetime" hint="Maximum 365 days."><div className="flex items-center gap-2"><input required type="number" min="1" max="365" value={ttl} onChange={(event) => setTtl(event.target.value)} className={`${inputClass} max-w-32`} /><span className="text-sm text-slate-500">days</span></div></Field><label className="flex items-start gap-2 text-sm text-slate-700"><input type="checkbox" checked={scoped} onChange={(event) => setScoped(event.target.checked)} className="mt-0.5 h-4 w-4 rounded border-slate-300 text-teal-600 focus:ring-teal-500" /><span><span className="font-medium">Limit this token to selected tools</span><span className="mt-0.5 block text-xs text-slate-500">Leave off to grant all {enabledTools.length} tools currently available to this owner, including enhanced knowledge and diagnostics capabilities.</span></span></label>{scoped ? <div className="grid max-h-48 gap-2 overflow-y-auto rounded-md border border-slate-200 bg-slate-50 p-3 sm:grid-cols-2">{enabledTools.map((tool) => <label key={tool.name} className="flex items-center gap-2 text-xs text-slate-700"><input type="checkbox" checked={selectedTools.includes(tool.name)} onChange={(event) => setSelectedTools((current) => event.target.checked ? [...current, tool.name] : current.filter((name) => name !== tool.name))} className="h-4 w-4 rounded border-slate-300 text-teal-600 focus:ring-teal-500" />{tool.name}</label>)}</div> : null}<div className="flex justify-end gap-2 border-t border-slate-200 pt-4"><button type="button" onClick={onCancel} className={secondaryButtonClass}>Cancel</button><button type="submit" disabled={isPending || !label.trim() || (scoped && !selectedTools.length)} className={primaryButtonClass}>{isPending ? 'Minting...' : 'Mint token'}</button></div></form>;
}

function TokenRow({ token, onEdit, onRevoke }) {
    const expired = token.status === 'expired';
    const visible = Number(token.visible_tool_count || 0);
    const eligible = Number(token.eligible_tool_count || 0);
    const accessLabel = eligible && visible < eligible ? `${visible} of ${eligible} tools` : `${visible} tools available`;
    return <div className="flex flex-col gap-3 py-4 lg:flex-row lg:items-center lg:justify-between"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><p className="font-semibold text-slate-900">{token.label}</p><StatusPill tone={expired ? 'neutral' : 'success'} label={expired ? 'Expired' : 'Active'} /></div><p className="mt-1 text-xs text-slate-500">{token.owner || 'Unknown owner'} · {ROLE_LABELS[token.role] || token.role || 'Unknown role'} · {accessLabel}</p><p className="mt-1 text-xs text-slate-400">Expires {formatDate(token.expires_at)} · Last used {formatDate(token.last_used_at, 'Never')}</p></div><div className="flex flex-wrap items-center gap-4 text-xs text-slate-500"><span><strong className="font-mono text-slate-800">{formatNumber(token.calls_7d)}</strong> calls / 7d</span><button type="button" onClick={onEdit} disabled={expired} className="font-semibold text-teal-700 hover:text-teal-800">Edit grant</button><button type="button" onClick={onRevoke} className="font-semibold text-rose-700 hover:text-rose-800">{expired ? 'Delete' : 'Revoke'}</button></div></div>;
}

function TokenGrantEditor({ token, tools, onCancel, onSubmit, isPending }) { const [selected, setSelected] = useState(() => (token?.abilities || []).filter((ability) => ability.startsWith('mcp:tool:')).map((ability) => ability.slice('mcp:tool:'.length))); const [identified, setIdentified] = useState(() => (token?.abilities || []).includes('mcp:identified-clients')); if (!token) return null; return <form onSubmit={(event) => { event.preventDefault(); onSubmit({ tools: selected, identified_clients: identified }); }} className="space-y-4"><p className="text-sm leading-6 text-slate-600">Changes are atomic and take effect on the same secret. The audit log records only a redacted grant diff.</p><div className="grid max-h-72 gap-2 overflow-y-auto rounded-md border border-slate-200 bg-slate-50 p-3 sm:grid-cols-2">{tools.filter((tool) => tool.enabled).map((tool) => <label key={tool.name} className="flex gap-2 text-xs text-slate-700"><input type="checkbox" checked={selected.includes(tool.name)} onChange={(event) => setSelected((current) => event.target.checked ? [...current, tool.name] : current.filter((name) => name !== tool.name))} className="mt-0.5 h-4 w-4 rounded border-slate-300 text-teal-600" />{tool.title || tool.name}</label>)}</div>{token.role === 'admin' ? <label className="flex items-start gap-2 rounded-md border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900"><input type="checkbox" checked={identified} onChange={(event) => setIdentified(event.target.checked)} className="mt-0.5 h-4 w-4" /><span><strong>Allow identified client rows</strong><span className="mt-1 block text-xs leading-5">Only with an explicit tool confirmation and purpose; all requests are audited.</span></span></label> : null}<div className="flex justify-end gap-2"><button type="button" onClick={onCancel} className={secondaryButtonClass}>Cancel</button><button type="submit" disabled={isPending} className={primaryButtonClass}>{isPending ? 'Saving...' : 'Save grant'}</button></div></form>; }
...
function Tools({ tools, activity, canManage, isSaving, onUpdate, previewMutation }) {
    ...
    const liveTools = tools.filter((tool) => tool.enabled).length;
    const rolloutTools = tools.filter((tool) => tool.management_mode === 'rollout').length;
    return <div className="space-y-4"><Section title="Tool registry" description={`${liveTools} of ${tools.length} capabilities are live. ${rolloutTools} enhanced capabilities require an explicit token permission; the rest follow the organisation registry.`} action={<button type="button" onClick={() => exportRowsToCsv('mcp-tools', toolCsvColumns, tools)} className={secondaryButtonClass}>Export CSV</button>}><div className="mb-4 border-b border-slate-200 pb-4"><p className="text-sm font-semibold text-slate-900">MCP Apps</p><p className="mt-1 text-sm text-slate-600">App-backed tools render inside compatible MCP clients. Select a tool marked App and run its preview here to verify the actual UI and data.</p></div><div className="flex flex-col gap-3 border-b border-slate-200 pb-4 sm:flex-row"><label className="sm:w-48"><span className="sr-only">Tool domain</span><select value={domain} onChange={(event) => setDomain(event.target.value)} className={inputClass}><option value="all">All domains</option>{Object.entries(DOMAIN_LABELS).map(([key, label]) => <option key={key} value={key}>{label}</option>)}</select></label><label className="sm:w-48"><span className="sr-only">Tool status</span><select value={status} onChange={(event) => setStatus(event.target.value)} className={inputClass}><option value="all">All statuses</option><option value="enabled">Live</option><option value="disabled">Unavailable</option></select></label></div><div className="grid gap-4 pt-4 lg:grid-cols-[minmax(0,1.35fr)_minmax(300px,0.8fr)]"><div className="overflow-hidden rounded-md border border-slate-200"><div className="hidden grid-cols-[minmax(220px,2fr)_110px_80px_80px_72px] gap-3 border-b border-slate-200 bg-slate-50 px-3 py-2 text-[10px] font-semibold uppercase tracking-[0.12em] text-slate-500 md:grid"><span>Tool</span><span>Minimum role</span><span>Calls / 7d</span><span>Avg ms</span><span>Status</span></div>{filtered.map((tool) => { const toolStats = activity.tool_stats?.[tool.name] || {}; const active = selected?.name === tool.name; return <button type="button" key={tool.name} onClick={() => setSelectedName(tool.name)} className={`grid w-full gap-2 border-b border-slate-200 px-3 py-3 text-left transition last:border-b-0 md:grid-cols-[minmax(220px,2fr)_110px_80px_80px_72px] md:items-center md:gap-3 ${active ? 'bg-teal-50/70' : 'bg-white hover:bg-slate-50'}`}><span className="min-w-0"><span className="flex flex-wrap items-center gap-2"><span className="truncate font-mono text-xs font-semibold text-slate-900">{tool.name}</span><span className="text-[10px] text-slate-400">{DOMAIN_LABELS[tool.domain] || tool.domain}</span>{tool.ui_resource ? <StatusPill tone="success" label="App" /> : null}{tool.scope_required ? <span className="text-[10px] font-semibold text-violet-700">Token permission</span> : null}</span><span className="mt-1 block text-xs text-slate-500">{tool.description}</span></span><span className="text-xs text-slate-500"><span className="md:hidden">Role: </span>{ROLE_LABELS[tool.configured_role] || tool.configured_role}</span><span className="font-mono text-xs text-slate-700"><span className="md:hidden">7d: </span>{formatNumber(toolStats.calls_7d)}</span><span className="font-mono text-xs text-slate-700"><span className="md:hidden">Avg: </span>{formatNumber(toolStats.avg_latency_ms)}</span><span><StatusPill tone={tool.enabled ? 'success' : 'neutral'} label={tool.enabled ? 'Live' : 'Off'} /></span></button>; })}</div>{selected ? <ToolInspector tool={selected} stats={stats} canManage={canManage} isSaving={isSaving} onToggle={() => onUpdate({ tools: { [selected.name]: { enabled: !selected.enabled, min_role: selected.configured_role } } })} onPreview={() => { previewMutation.reset(); setPreviewOpen(true); }} /> : <EmptyState title="No tools match" message="Change the domain or status filters." />}</div></Section><McpDialog open={previewOpen} title={`Preview ${selected?.name || 'tool'}`} onClose={() => setPreviewOpen(false)}><ToolPreviewForm tool={selected} previewMutation={previewMutation} onClose={() => setPreviewOpen(false)} /></McpDialog></div>;
}

function ToolInspector({ tool, stats, canManage, isSaving, onToggle, onPreview }) {
    return <aside className="rounded-md border border-slate-200 bg-slate-50 p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="font-mono text-sm font-semibold text-slate-950">{tool.name}</p><p className="mt-1 text-sm leading-6 text-slate-600">{tool.description}</p></div><StatusPill tone={tool.enabled ? 'success' : 'neutral'} label={tool.enabled ? 'Live' : 'Off'} /></div>{tool.ui_resource ? <div className="mt-4 border-l-2 border-teal-600 pl-3"><p className="text-sm font-semibold text-slate-800">Interactive MCP App</p><p className="mt-1 text-xs leading-5 text-slate-600">A compatible MCP client opens this dashboard from the tool result. Preview it here to confirm the rendered view.</p><code className="mt-2 block break-all font-mono text-[11px] text-slate-600">{tool.ui_resource}</code></div> : null}<div className="mt-4 grid grid-cols-2 gap-2"><MiniFact label="Min role" value={ROLE_LABELS[tool.configured_role] || tool.configured_role} /><MiniFact label="Calls / 7d" value={formatNumber(stats.calls_7d)} /><MiniFact label="Average" value={`${formatNumber(stats.avg_latency_ms)} ms`} /><MiniFact label="Errors" value={formatNumber(stats.errors_7d)} /></div><div className="mt-4 border-t border-slate-200 pt-4"><p className="text-[10px] font-semibold uppercase tracking-[0.12em] text-slate-400">Arguments</p><pre className="mt-2 max-h-32 overflow-auto rounded-md bg-white p-3 font-mono text-[11px] leading-5 text-slate-700">{JSON.stringify(tool.inputSchema?.properties || {}, null, 2)}</pre></div><div className="mt-4"><p className="text-[10px] font-semibold uppercase tracking-[0.12em] text-slate-400">Reads</p><p className="mt-2 text-xs text-slate-600">{tool.backing_service}{tool.views?.length ? ` · ${tool.views.join(', ')}` : ''}</p><p className="mt-3 text-[10px] font-semibold uppercase tracking-[0.12em] text-slate-400">Returns no</p><p className="mt-2 text-xs leading-5 text-slate-600">{(tool.returns_no || []).join(' · ')}</p></div>{tool.scope_required ? <p className="mt-4 rounded-md border border-violet-200 bg-violet-50 px-3 py-2 text-xs leading-5 text-violet-900">This enhanced capability is live, but each token must be granted this permission when it is minted.</p> : null}<p className="mt-3 text-xs text-slate-500">{tool.status_detail}</p><div className="mt-4 flex flex-wrap gap-2"><button type="button" onClick={onPreview} disabled={!tool.enabled} className={secondaryButtonClass}>{tool.ui_resource ? 'Preview app' : 'Preview payload'}</button>{tool.management_mode === 'registry' ? <button type="button" onClick={onToggle} disabled={!canManage || isSaving} className={`${buttonClass} ${tool.enabled ? 'border border-rose-200 bg-white text-rose-700 hover:bg-rose-50' : 'bg-teal-700 text-white hover:bg-teal-800'}`}>{tool.enabled ? 'Disable tool' : 'Enable tool'}</button> : null}</div>{!canManage ? <p className="mt-3 text-xs text-amber-700">Admin permission is required to change the registry.</p> : null}</aside>;
}
...
function ToolPreviewForm({ tool, previewMutation, onClose }) {
    ...
    return <div className="space-y-4"><p className="text-sm text-slate-600">This executes the selected tool with the same governance and sanitizer used by a real client, then discards the result after previewing it.</p><label className="block"><span className="mb-1 block text-sm font-medium text-slate-700">Arguments JSON</span><textarea value={argsText} onChange={(event) => setArgsText(event.target.value)} className={`${inputClass} min-h-28 font-mono text-xs`} spellCheck="false" /></label>{error ? <p className="text-sm text-rose-700">{error}</p> : null}<div className="flex justify-end gap-2"><button type="button" onClick={onClose} className={secondaryButtonClass}>Close</button><button type="button" onClick={submit} disabled={previewMutation.isPending || !tool.enabled} className={primaryButtonClass}>{previewMutation.isPending ? 'Running...' : 'Run preview'}</button></div>{result ? <div className="rounded-md border border-emerald-200 bg-emerald-50 p-3"><div className="flex flex-wrap items-center justify-between gap-2"><p className="font-semibold text-emerald-900">PII scan passed</p><span className="text-xs text-emerald-800">{formatBytes(result.bytes)} · {formatNumber(result.row_count)} rows</span></div>{result.app_html ? <McpAppPreview html={result.app_html} payload={result.payload} /> : null}<details className="mt-3"><summary className="cursor-pointer text-xs font-semibold text-emerald-900">View JSON response</summary><pre className="mt-2 max-h-72 overflow-auto rounded bg-white/70 p-3 font-mono text-[11px] leading-5 text-slate-800">{JSON.stringify(result.payload, null, 2)}</pre></details><p className="mt-2 text-xs text-emerald-800">Checked: {(result.pii_scan?.fields_checked || []).join(', ')}.</p></div> : null}</div>;
}

function McpAppPreview({ html, payload }) {
    const frame = useRef(null);
    const sendResult = () => frame.current?.contentWindow?.postMessage({ jsonrpc: '2.0', method: 'ui/notifications/tool-result', params: { result: payload } }, '*');
    useEffect(sendResult, [payload]);
    return <iframe ref={frame} title="Revenue dashboard app preview" sandbox="allow-scripts" srcDoc={html} onLoad={sendResult} className="mt-3 h-[420px] w-full rounded-md border border-slate-200 bg-white" />;
}

function Privacy({ settings, tools, canManage, isSaving, onSave, previewMutation, staff, aliasesLoading, onSaveAlias, aliasSaving }) {
...
function Guide({ endpoint, tools, selfTest, onSelfTest, isSelfTesting }) {
    ...
    const codex = `[mcp_servers.exotic]\nurl = "${endpoint}"\nbearer_token_env_var = "EXOTIC_MCP_TOKEN"`;
    const examples = [['Show the revenue dashboard for the last 30 days', 'render_revenue_dashboard (opens an MCP App in supported clients)'], ['Why did new-user revenue fall this window?', 'revenue_summary -> market_breakdown -> churn_analysis'], ['Which markets have profiles expiring in 7 days?', 'lifecycle_summary'], ['What errors started after the last deploy?', 'error_digest -> system_vitals']];
    ...
    return <div className="space-y-4"><Section title="Connect a remote assistant" description="ChatGPT and Claude connect through CRM-hosted OAuth with PKCE. The connector receives only the read-only capabilities available to the CRM account that approves it."><div className="grid gap-3 lg:grid-cols-3"><ConnectionCard title="ChatGPT" steps={['Enable Developer Mode where your workspace allows it.', 'Apps → Create → paste the endpoint below.', 'Choose OAuth, scan tools, then sign in and approve the CRM consent screen.']} endpoint={endpoint} /><ConnectionCard title="Claude remote" steps={['Team/Enterprise: Organization settings → Connectors → Add → Custom → Web.', 'Pro/Max: Customize → Connectors → Add custom connector.', 'Paste the endpoint and complete the CRM consent screen.']} endpoint={endpoint} /><ConnectionCard title="Claude Code (optional)" steps={['Use a managed bearer token only for technical local setup.', 'Mint a scoped credential in Tokens.', 'Keep the token in your local environment, never in source control.']} endpoint={endpoint} technical /></div></Section><div className="grid gap-4 xl:grid-cols-[minmax(0,1.35fr)_minmax(300px,0.8fr)]"><div className="space-y-4"><Section title="Technical fallback" description="Claude Code remains available where a remote connector is not appropriate."><div className="flex flex-wrap gap-1 border-b border-slate-200"><button type="button" onClick={() => setClient('claude')} className={`${buttonClass} ${client === 'claude' ? 'border-b-2 border-teal-700 text-slate-900' : 'text-slate-500'}`}>Claude Code</button><button type="button" onClick={() => setClient('codex')} className={`${buttonClass} ${client === 'codex' ? 'border-b-2 border-teal-700 text-slate-900' : 'text-slate-500'}`}>Codex CLI</button></div><CommandBlock label="Local connection" value={setupCommand} /></Section><Section title="Ask something useful" description={`${tools.filter((tool) => tool.enabled).length} tools are currently visible. Write a question, then copy it into your connected client.`}><textarea value={question} onChange={(event) => setQuestion(event.target.value)} className={`${inputClass} min-h-24 resize-y`} placeholder="Ask about revenue, lifecycle, markets or system health" /><div className="mt-3 flex justify-end"><button type="button" onClick={copyQuestion} disabled={!question.trim()} className={primaryButtonClass}>{copied ? 'Prompt copied' : 'Copy prompt'}</button></div></Section></div><div className="space-y-4"><Section title="Verify connection" action={<button type="button" onClick={onSelfTest} disabled={isSelfTesting} className={secondaryButtonClass}>{isSelfTesting ? 'Running...' : 'Run self-test'}</button>}>{selfTest?.checks?.length ? <div className="space-y-2">{selfTest.checks.map((check) => <div key={check.key} className="flex gap-2 text-sm"><StatusDot ok={check.status === 'ok'} /><span className="text-slate-700">{check.message}</span></div>)}</div> : <p className="text-sm text-slate-500">Run the self-test to verify the endpoint, registry and database path.</p>}</Section><Section title="Privacy boundary"><p className="text-sm leading-6 text-slate-600">MCP never writes, returns phone/email/payment references/message content, or exposes Support Board. Named client rows require an administrator-only capability, purpose and explicit confirmation.</p></Section></div></div></div>;
}

function ConnectionCard({ title, steps, endpoint, technical = false }) { return <article className="rounded-md border border-slate-200 bg-slate-50 p-4"><div className="flex items-center justify-between gap-3"><h3 className="font-semibold text-slate-900">{title}</h3><StatusPill tone={technical ? 'neutral' : 'success'} label={technical ? 'Optional' : 'OAuth + PKCE'} /></div><ol className="mt-3 space-y-2 text-sm leading-5 text-slate-600">{steps.map((step, index) => <li key={step} className="flex gap-2"><span className="font-mono text-xs text-teal-700">0{index + 1}</span><span>{step}</span></li>)}</ol><div className="mt-4 flex items-center gap-2"><code className="min-w-0 flex-1 truncate rounded bg-white px-2 py-1 font-mono text-[11px] text-slate-600">{endpoint}</code><CopyButton value={endpoint} label="Copy" /></div></article>; }
