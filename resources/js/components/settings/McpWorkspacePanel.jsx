...
function TokenRow({ token, onRevoke }) {
    ...
    return <div className="flex flex-col gap-3 py-4 lg:flex-row lg:items-center lg:justify-between"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><p className="font-semibold text-slate-900">{token.label}</p><StatusPill tone={expired ? 'neutral' : 'success'} label={expired ? 'Expired' : 'Active'} /></div><p className="mt-1 text-xs text-slate-500">{token.owner || 'Unknown owner'} · {ROLE_LABELS[token.role] || token.role || 'Unknown role'} · {accessLabel}</p>{eligible && visible < eligible ? <p className="mt-1 text-xs text-amber-700">This token is missing {eligible - visible} available capabilities. Mint a replacement with full access to include them.</p> : null}<p className="mt-1 text-xs text-slate-400">Expires {formatDate(token.expires_at)} · Last used {formatDate(token.last_used_at, 'Never')}</p></div><div className="flex flex-wrap items-center gap-4 text-xs text-slate-500"><span><strong className="font-mono text-slate-800">{formatNumber(token.calls_7d)}</strong> calls / 7d</span><span><strong className="font-mono text-slate-800">{formatBytes(token.bytes_7d)}</strong> out</span><span><strong className="font-mono text-slate-800">{formatCostRange(token.bytes_7d)}</strong> rough cost</span><button type="button" onClick={onRevoke} className="font-semibold text-rose-700 hover:text-rose-800">{expired ? 'Delete' : 'Revoke'}</button></div></div>;
}
