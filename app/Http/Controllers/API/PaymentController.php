...
class PaymentController extends Controller
{
    ...
    {
        $kopokopoSignature = trim((string) $request->header('X-KopoKopo-Signature', ''));
        if ($kopokopoSignature !== '') {
            try {
                $result = app(BillingGatewayService::class)->handleMpesaCallback(
    ...
    public function debugKopokopo()
    {
        return response()->json([
            ...
            'config' => [
                ...
                'callback_url' => url('/api/billing/mpesa/callback'),
                'legacy_callback_url' => url('/api/payment-callback'),
    ...
    public function manualActivate(Request $request)
    {
        ...
        try {
            ...
            if ((int) $product->platform_id !== 0 && (int) $product->platform_id !== (int) $platformId) {
                return response()->json([
                    'error' => 'Activation failed',
                    'message' => 'The selected package does not belong to this platform.',
                ], 422);
            }
    ...
    public function handleNotification(Request $request)
    {
        try {
            ...
            Log::warning('CyberSource notification rejected', [
            ...
            Log::warning('CyberSource notification payload rejected', [
                'reason' => $exception->getMessage(),
                'payload' => $request->except(['signature']),
            ]);
    ...
    public function paymentCancel(Request $request)
    {
        ...
        if (
            $payment
            && $this->hasValidCybersourceSignature($request)
            && $this->normalizePaymentStatus($payment->status) !== 'completed'
        ) {
            $payment->forceFill([
                'status' => 'canceled',
            ...
            Log::warning('CyberSource cancel ignored because signature was missing or invalid', [
                'payment_id' => $payment->id,
                'transaction_uuid' => $payment->transaction_uuid,
                'payload' => $request->except(['signature']),
            ]);
    ...
    {
        $this->ensureValidCybersourceSignature($request);

        $signedFields = $this->cybersourceSignedFields($request);
        $this->ensureRequiredCybersourceFieldsAreSigned($request, $signedFields);

        $decision = strtoupper((string) $request->input('decision', ''));
        $status = $this->mapDecisionToStatus($decision !== '' ? $decision : 'ERROR');
        $payment = $this->resolveSignedCybersourcePayment($request, $signedFields);
        $reason = (string) (
        ...
        $this->ensureCybersourcePaymentFactsMatch($request, $payment, $signedFields);
        $currentStatus = $this->normalizePaymentStatus($payment->status);

        if ($status === 'completed' && ! in_array($currentStatus, ['pending', 'initiated', 'under_review'], true)) {
            throw new InvalidArgumentException('Payment is not eligible for CyberSource completion.');
        }
    ...
    private function ensureValidCybersourceSignature(Request $request): void
    {
        if (! $this->hasValidCybersourceSignature($request)) {
            throw new UnexpectedValueException('Invalid CyberSource signature.');
        }
    }

    private function hasValidCybersourceSignature(Request $request): bool
    {
        $secret = (string) config('services.cybersource.secret_key', '');
        $signature = trim((string) $request->input('signature', ''));
        $signedFieldNames = trim((string) $request->input('signed_field_names', ''));

        if ($secret === '' || $signature === '' || $signedFieldNames === '') {
            return false;
        }

        $fields = array_values(array_filter(array_map('trim', explode(',', $signedFieldNames))));

        if ($fields === [] || in_array('signature', $fields, true)) {
            return false;
        }
        ...
        foreach ($fields as $field) {
            ...
            $signedValues[] = $field.'='.(string) $request->request->get($field);
        }

        $expected = base64_encode(hash_hmac('sha256', implode(',', $signedValues), $secret, true));

        return hash_equals($expected, $signature);
    }

    private function cybersourceSignedFields(Request $request): array
    {
        $fields = array_values(array_filter(array_map(
            'trim',
            explode(',', (string) $request->input('signed_field_names', ''))
        )));

        return array_fill_keys($fields, true);
    }

    private function ensureRequiredCybersourceFieldsAreSigned(Request $request, array $signedFields): void
    {
        if (! isset($signedFields['decision'])) {
            throw new InvalidArgumentException('CyberSource decision was not signed.');
        }

        $hasSignedPaymentIdentifier = $this->firstSignedCybersourceInput($request, $signedFields, [
            'transaction_uuid',
        ...
        if (! $hasSignedPaymentIdentifier) {
            throw new InvalidArgumentException('CyberSource payment identifier was not signed.');
        }

        $hasSignedProviderIdentifier = $this->firstSignedCybersourceInput($request, $signedFields, [
            'request_id',
            'transaction_id',
            'transaction_uuid',
        ]) !== null;

        if (! $hasSignedProviderIdentifier) {
            throw new InvalidArgumentException('CyberSource transaction identifier was not signed.');
        }
    }

    private function resolveSignedCybersourcePayment(Request $request, array $signedFields): ?Payment
    {
        $identifier = $this->firstSignedCybersourceInput($request, $signedFields, [
            'transaction_uuid',
            'req_reference_number',
            'reference_number',
            'payment_id',
        ...
        return $this->resolveLegacyBrowserPayment($identifier);
    }

    private function ensureCybersourcePaymentFactsMatch(Request $request, Payment $payment, array $signedFields): void
    {
        $providerKey = trim((string) ($payment->provider_key ?? ''));
        if ($providerKey !== '' && ! in_array(strtolower($providerKey), ['cybersource', 'card', 'legacy_card'], true)) {
            throw new InvalidArgumentException('Payment is not eligible for CyberSource mutation.');
        }

        $reference = $this->firstSignedCybersourceInput($request, $signedFields, [
            'req_reference_number',
            'reference_number',
        ]);

        if ($reference !== null) {
            $allowedReferences = array_filter(array_map('strval', [
            ...
            if (! in_array($reference, $allowedReferences, true)) {
                throw new InvalidArgumentException('CyberSource reference does not match payment.');
            }
        }

        $amount = $this->firstSignedCybersourceInput($request, $signedFields, [
            'auth_amount',
            'req_amount',
            'amount',
            'decision_amount',
        ...
        if ($amount !== null && is_numeric($amount) && abs((float) $amount - (float) $payment->amount) > 0.01) {
            throw new InvalidArgumentException('CyberSource amount does not match payment.');
        }

        $currency = $this->firstSignedCybersourceInput($request, $signedFields, [
            'auth_currency',
            'req_currency',
            'currency',
        ]);
        ...
        if ($currency !== null && strtoupper($currency) !== strtoupper((string) ($payment->currency ?? ''))) {
            throw new InvalidArgumentException('CyberSource currency does not match payment.');
        }
    }

    private function firstSignedCybersourceInput(Request $request, array $signedFields, array $fields): ?string
    {
        foreach ($fields as $field) {
            if (isset($signedFields[$field]) && $request->request->has($field)) {
