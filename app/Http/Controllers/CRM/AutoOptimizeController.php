...
class AutoOptimizeController extends Controller
{
    ...
    private function validatedPayload(Request $request, ?AutoOptimizePlan $plan = null): array
    {
        $validated = $request->validate([
            ...
            'actions.generation.bio_format' => ['nullable', 'string', Rule::in(array_keys(\App\Services\Seo\BioGenerationService::BIO_FORMATS))],
            'actions.generation.creativity' => 'nullable|numeric|min:0|max:1.4',
            'actions.generation.overuse_sensitivity' => 'nullable|string|in:low,medium,high',
