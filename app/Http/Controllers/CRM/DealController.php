    ...
    public function activate(Request $request, Deal $deal)
    {
        ...
        try {
            ...
            AuditLog::create([
                ...
                'reason' => $request->input('reason', 'Activated via CRM'),
    ...
    public function extend(Request $request, Deal $deal)
    {
        ...
        try {
            ...
            AuditLog::create([
                'platform_id' => $client->platform_id,
                'actor_id' => $request->user()->id,
                'action' => 'deal_extended',
                'entity_type' => 'deal',
                'entity_id' => $deal->id,
                'before_state' => $beforeState,
                'after_state' => ['expires_at' => $newExpiry->toDateTimeString()],
                'reason' => $request->input('reason', 'Extended via CRM'),
                'ip_address' => $request->ip(),
                'created_at' => now(),
            ]);
